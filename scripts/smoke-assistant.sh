...
for pod in $(kubectl get pod -l app=frontend --field-selector=status.phase=Running -o name); do
  ...
  for c in "en|Do you have any men's polo shirts?" "ja|メンズのポロシャツはありますか？"; do
    lang=${c%%|*}; q=${c#*|}
    body=$(python3 -c 'import json,sys; print(json.dumps({"message":sys.argv[1],"history":[],"lang":sys.argv[2],"nocache":True}))' "$q" "$lang")
    verdict=$(curl -s -m 120 -X POST "http://127.0.0.1:$port/bot" -H 'Content-Type: application/json' -d "$body" | python3 /tmp/smoke-check.py "$lang")
    rc=$?
    echo "${pod#pod/} [$img] UI=$lang -> $verdict"
    if [ $rc -ne 0 ]; then
      echo "::error::${pod#pod/} UI=$lang: $verdict"
      fail=1
    fi
  done
